...
--Agrupar Quantidade de Vendas dos Funcionário por Dia (Não OK - Coluna não definida no agrupamento)(ERRO)
...
--Agrupar Quantidade de Vendas dos Funcionário por Dia (OK)(CORRIGIDO)
SELECT  SalesPersonID AS ID_do_Vendedor, OrderDate AS Data_da_Venda,
		COUNT(SalesOrderID) AS Total_de_Vendas
FROM Sales.SalesOrderHeader
WHERE SalesPersonID IS NOT NULL
...
--Função de Agregação x Outras Funções no SELECT (ERRO)
--Agrupar Quantidade de Vendas dos Funcionário por Ano (Não OK - coluna com função de data não definida no agrupamento)
...
--Agrupar Quantidade de Vendas dos Funcionário por Ano (OK)(CORRIGIDO)
SELECT  SalesPersonID AS ID_do_Vendedor, YEAR(OrderDate) AS Ano_da_Venda,
		COUNT(SalesOrderID) AS Total_de_Vendas
FROM Sales.SalesOrderHeader
...
--Alias no Group By
--Valor Total de Vendas Por Dia (ERRO)
...
--Valor Total de Vendas Por Dia (OK) (CORRIGIDO)
SELECT OrderDate AS Data_da_Venda, SUM(TotalDue) AS Valor_Total_de_Vendas
FROM Sales.SalesOrderHeader
GROUP BY OrderDate
ORDER BY 1 ASC;
...
--Clientes que Fizeram mais de 10 compras em 2013: (ERRO)
