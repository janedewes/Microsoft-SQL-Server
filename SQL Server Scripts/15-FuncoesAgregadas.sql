...
--Funções SUM e AVG (soma e média)
...
--Valores NULOS (<> de branco) e Funções de Agregação (nulo é ausencia de valor!)
...
--Tratando Valores Nulos - para fazer a troca, quando aparecer nulo trocar por zero (nao atualiza o BD)
SELECT	Weight AS "Sem Peso", COALESCE (Weight,0) AS "Peso Zerado"
FROM Production.Product;


-- Verificar se existem valores nulos na tabela para a coluna Weight
SELECT COUNT(*)
FROM Production.Product
WHERE Weight IS NULL

-- Verificar as cols dessa tabela
